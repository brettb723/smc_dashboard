...
def display_report():
    ...
    for owner, group in df.groupby('Owners'):
        # Initialize a new row with the owner's name and zeros for each bucket
        new_row = {'Owners': owner, **{bucket: 0 for bucket in aging_buckets}}
        ...
        aging_report_df = pd.concat([aging_report_df, pd.DataFrame([new_row])], ignore_index=True)

    # Calculate totals for each column (excluding the 'Owners' column)
    totals = aging_report_df.drop('Owners', axis=1).sum()
    totals['Owners'] = 'Total'  # Add 'Owners' column with 'Total' as the value
    # Append totals row to the DataFrame
    aging_report_df = pd.concat([aging_report_df, pd.DataFrame([totals])], ignore_index=True)
